	...
	return (
		<Pagination
			...
			showTotal={(total, range) => {
				...
				if (tbl_wrapper) {
					let span_page_from = tbl_wrapper.querySelector(".span_page_from");
					let span_page_to = tbl_wrapper.querySelector(".span_page_to");
					let span_page_total = tbl_wrapper.querySelector(".span_page_total");
					if (span_page_from) {
					...
					let span_page_from = document.querySelector(".span_page_from");
					let span_page_to = document.querySelector(".span_page_to");
					let span_page_total = document.querySelector(".span_page_total");
					if (span_page_from) {
						span_page_from.textContent = range[0];
					}
					if (span_page_to) {
...
export function TableGlobalFilter(props) {
	const {
		tableFilter,
		setTableFilter,
		filter_key_name,
	...
	return (
		<div className="tbl-select-wrapper">
			<Select
				placeholder={placeholder ?? "Select Department"}
				size={size ?? "large"}
				className={className ?? "ant-input-padding-inherit br-0 "}
				options={options}
				onChange={(e) => {
					setTableFilter((ps) => ({
						...ps,
...
export function TableGlobalYearFilter(props) {
	const { tableFilter, setTableFilter, className, option, placeholder, size } =
		props;

	const { RangePicker } = DatePicker;

	return (
		<div className="tbl-year-wrapper">
			<RangePicker
				picker="year"
				id={{
					start: "startInput",
